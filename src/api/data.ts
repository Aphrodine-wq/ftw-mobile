...
/**
 * In dev mode, fall back to mock data when API fails.
 * In production, throw so the UI can show a real error.
 */
function fallback<T>(mock: T): T {
...
export async function fetchJobs() {
  ...
  } catch (e) { if (!__DEV__) throw e; }
  return fallback(mockJobs);
}
...
export async function fetchNotifications() {
  ...
  return fallback(mockNotifications);
}
...
export async function fetchMySubJobs() {
  ...
  return fallback(mockSubJobs.filter((sj) => sj.status === "in_progress" || sj.status === "completed"));
}
...
export async function fetchSubContractorStats() {
  ...
  return fallback(subContractorStats);
}
