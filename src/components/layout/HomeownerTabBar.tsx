...
const TABS = [
  {
    ...
    route: "/(homeowner)/(dashboard)" as const,
    match: ["/(homeowner)/(dashboard)", "/(dashboard)"],
  },
  {
    key: "jobs",
    label: "My Jobs",
    icon: Briefcase,
    route: "/(homeowner)/(jobs)" as const,
    match: ["/(homeowner)/(jobs)", "/(jobs)"],
  },
  {
    key: "action",
    label: "",
    icon: Plus,
    route: "/(homeowner)/post-job" as const,
    match: ["/(homeowner)/post-job", "/post-job"],
    isCenter: true,
  },
  {
    key: "messages",
    label: "Messages",
    icon: MessageCircle,
    route: "/(homeowner)/(messages)" as const,
    match: ["/(homeowner)/(messages)", "/(messages)"],
  },
  {
    key: "profile",
    label: "Profile",
    icon: User,
    route: "/(homeowner)/(profile)" as const,
    match: ["/(homeowner)/(profile)", "/(profile)"],
  },
] as const;

const FEATURED_ACTIONS = [
  { key: "post-job", label: "Post a Job", icon: ClipboardList, route: "/(homeowner)/post-job" },
] as const;

const QUICK_ACTIONS = [
  { key: "messages", label: "Messages", icon: MessageCircle, route: "/(homeowner)/(messages)" },
  { key: "projects", label: "Projects", icon: FolderOpen, route: "/(homeowner)/projects" },
  { key: "reviews", label: "Reviews", icon: Star, route: "/(homeowner)/reviews" },
  { key: "notifications", label: "Notifications", icon: Bell, route: "/(homeowner)/notifications" },
  { key: "settings", label: "Settings", icon: Settings, route: "/(homeowner)/settings" },
] as const;
